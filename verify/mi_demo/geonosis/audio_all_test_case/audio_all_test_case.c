...
MI_AI_AecConfig_t stAecCfg = {
    ...
    .u32AecRatioThreshold = 0,
    .u32AecDistortionTestFreq = {0, 0},
};
...
void display_help(void)
{
	...
	printf("AI Device Id: Amic[0] Dmic[1] I2S RX[2] Linein[3] Amic+I2S RX[4] Dmic+I2S RX[5] Amic+Dmic[6]\n");
...
MI_BOOL checkParam(void)
{
    ...
    do{
        ...
        if (bEnableAI)
        {
            ...
            else if (AI_DEV_AMIC_AND_DMIC == AiDevId)
            {
                if ((E_AI_SOUND_MODE_MONO == eAiSoundMode)
					|| (E_AI_SOUND_MODE_QUEUE == eAiSoundMode))
			    {
                    if (AI_AMIC_AND_DMIC_CHN_MAX != u32AiChnCnt)
                    {
                        printf("Ai channel count invalid!!!\n");
	                    break;
                    }
			    ...
			    {
                    if (AI_AMIC_AND_DMIC_CHN_MAX / 2 != u32AiChnCnt)
                    {
                        printf("Ai channel count invalid!!!\n");
	                    break;
                    }
...
MI_S32 createOutputFileName(AiOutFileName_t *pstAiOutFileName, MI_U32 u32ChnIdx)
{
    ...
    else if (AI_DEV_AMIC_AND_DMIC == pstAiOutFileName->AiDevId)
    {
        sprintf(as8Tmp, "Amic+Dmic_" );
    }
    strcat((char *)pstAiOutFileName->ps8OutputFile, as8Tmp);
...
MI_S32 initAi(void)
{
    ...
    for (u32ChnIdx = 0; u32ChnIdx < u32ChnCnt; u32ChnIdx++)
    {
        ...
        ExecFunc(MI_SYS_SetChnOutputPortDepth(&stAiChnOutputPort0[u32ChnIdx], 4, TOTAL_BUF_DEPTH), MI_SUCCESS);
    }
